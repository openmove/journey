    ...
    return (
      <>
        ...
        <div className="otp-ui-transitLegBody">
          ...
          {/* Agency information */}
          {showAgencyInfo && (
            <div className="otp-ui-transitLegBody-agencyInfo">
              <p className="subtitle">{t('service')}</p>
              <Button bsSize="small" bsStyle="link" href={agencyUrl} rel="noopener noreferrer" target="_blank">
                {agencyName}
                {logoUrl && (
                  <img alt={`${agencyName} logo`} src={logoUrl} height={23} style={{marginLeft: 8, verticalAlign:'text-bottom'}}/>
                )}
              </Button>
            </div>
          )}
